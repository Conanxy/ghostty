...
extension Ghostty {
    ...
    class Config: ObservableObject {
        ...
        var macosTabBarPosition: MacOSTabBarPosition {
            let defaultValue = MacOSTabBarPosition.top
            guard let config = self.config else { return defaultValue }
            var v: UnsafePointer<Int8>?
            let key = "macos-tab-bar-position"
            guard ghostty_config_get(config, &v, key, UInt(key.lengthOfBytes(using: .utf8))) else { return defaultValue }
            guard let ptr = v else { return defaultValue }
            return MacOSTabBarPosition(rawValue: String(cString: ptr)) ?? defaultValue
        }
...
extension Ghostty.Config {
    ...
    enum MacOSTabBarPosition: String {
        case top, left, right
    }
}
